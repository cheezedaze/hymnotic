...
import ConfirmNewsletter from "@/emails/ConfirmNewsletter";
...
export async function sendInvitationEmail(
  ...
  await resend.emails.send({
    from: fromEmail,
    to: email,
    subject: "You're invited to HYMNZ",
    html: `
      <div style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; max-width: 500px; margin: 0 auto; padding: 40px 20px; color: #e0e0e0; background-color: #141A24;">
        ...
        <p style="font-size: 15px; line-height: 1.6; color: #b0b0b0;">
          ${inviterName ? `${inviterName} has invited you` : "You've been invited"} to join HYMNZ &mdash; a sacred music experience.
        </p>
...
export async function sendNewsletterConfirmEmail(
  ...
  await resend.emails.send({
    ...
    // Rendered here rather than passed as `react` — the SDK's React path needs
